...
export class migrations1675933116633 implements MigrationInterface {
  name = 'migrations1675933116633';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE \`Channel\` (\`id\` int NOT NULL AUTO_INCREMENT, \`channel_id\` varchar(255) NOT NULL, \`channel_name\` varchar(255) NOT NULL, \`url\` varchar(255) NOT NULL, \`subscriber\` int NOT NULL, \`description\` varchar(255) NOT NULL, \`since\` datetime NOT NULL, \`total_views\` int NOT NULL, \`total_videos\` int NOT NULL, \`country\` varchar(255) NOT NULL, \`link\` varchar(255) NOT NULL, \`keyword\` varchar(255) NOT NULL, \`total_normal_videos\` int NOT NULL, \`tag\` varchar(255) NOT NULL, \`userId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `CREATE TABLE \`User\` (\`id\` int NOT NULL AUTO_INCREMENT, \`user_email\` varchar(255) NOT NULL, \`channel_id\` varchar(255) NOT NULL, \`token_refresh\` varchar(255) NOT NULL, \`agree_promotion\` varchar(255) NOT NULL, \`plan\` varchar(255) NOT NULL, \`is_admin\` tinyint NOT NULL, \`status\` varchar(255) NOT NULL, \`date_sign_in\` datetime NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE \`Channel\` ADD CONSTRAINT \`FK_5a8718b3a93d2a6cd2a955eb56a\` FOREIGN KEY (\`userId\`) REFERENCES \`User\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`Channel\` DROP FOREIGN KEY \`FK_5a8718b3a93d2a6cd2a955eb56a\``,
    );
    await queryRunner.query(
